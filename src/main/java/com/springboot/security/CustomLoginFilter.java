...
public class CustomLoginFilter extends AbstractAuthenticationProcessingFilter {
	...
	protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain,
		...
		sessionDto.setProfileImgUrl(user.getProfileImgUrl());
		
		if(user.getType().equals(UserType.CUSTOMER)) {
			
			Order latestOrder = orderService.getCurrentByCustomerId(user.getId());
			log.debug("latestOrder{}",ObjectUtils.toJson(latestOrder));
			if(latestOrder!=null) {
				sessionDto.setLatestOrder(entityMapper.mapOrderToOrderReadDTO(latestOrder));
			}
